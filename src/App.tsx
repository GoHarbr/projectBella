...
function App() {
  ...
  const [questions, setQuestions] = useState({
    entityBreakdown: true,
    concentration: true,
    concentrationCategory: true,
  });
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (questions.concentration) {
        promptQuestions.push('Which entity holds the greatest payable or receivable concentration. Also express the concentration as a percentage versus the total.');
      }
      if (questions.concentrationCategory) {
        promptQuestions.push('What are the aging categories? and which category has the greatest concentration.');
      }
      if (questions.entityBreakdown) {
        promptQuestions.push('Breakdown the report by entity in order of highest concentration. Show this as a table. Provide insights.');
      }
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="inline-flex items-center">
                <input
                  type="checkbox"
                  checked={questions.entityBreakdown}
                  onChange={(e) =>
                    setQuestions({
                      ...questions,
                      entityBreakdown: e.target.checked,
                    })
                  }
              ...
              <label className="inline-flex items-center">
                <input
                  ...
                  className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                />
                <span className="ml-2 text-sm text-gray-600">Concentration Entity</span>
              </label>
              <br /> 
              <label className="inline-flex items-center">
                <input
                  type="checkbox"
                  checked={questions.concentrationCategory}
                  onChange={(e) =>
                    setQuestions({
                      ...questions,
                      concentrationCategory: e.target.checked,
                    })
                  }
                  className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                />
